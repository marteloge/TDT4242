 ...
 * @author Rune Molden
 * @version $Revision: 1.8 $ - $Date: 2008-04-24 11:18:21 $
 */
...
//	    StringTokenizer tokenizer = new StringTokenizer(dateFormatted);
	...
	private void saveFlatFile(Project aProject, File aFile) throws IOException {
		...
		for (int i = 0; i < aProject.getPersonCount(); i++) {
			printWriter.println(aProject.getPerson(i).getVehicleID() + ";" + 
					aProject.getPerson(i).getName() + ";" +
					aProject.getPerson(i).getEmail() + ";" + 
			        aProject.getPerson(i).getCity() + ";" + 
			        aProject.getPerson(i).getStreet() + ";" +
			        aProject.getPerson(i).getVehicleID());
	...
	private Person assemblePerson(String line) throws ParseException {
		...
		return new Person(Integer.parseInt(custId), name, email, street, city, vehicleID);
	}	
}
